...
function WhyChoose() {
  ...
  return (
    <section className="w-full bg-white px-6 lg:px-16 py-5">
      ...
      <p className="flex justify-center text-center text-gray-500 text-[17px] mx-auto mb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {features.map((f) => (
          <div key={f.title} className="flex justify-center items-center bg-white rounded-xl p-8 flex flex-col gap-4">
            ...
            <h3 className="font-bold text-gray-800 text-[17px]">{f.title}</h3>
            <p className="text-gray-500 text-[12px] leading-relaxed text-center">{f.desc}</p>
          </div>
        ))}
      </div>
    </section>
  ...
  return (
    <section className="w-full bg-white px-6 lg:px-16 py-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-6 gap-3 my-8">
        {cats.map((cat) => (
          <div key={cat} className="border border-black-200 rounded-md px-2 py-2 text-center text-bold  text-[17px] text-black cursor-pointer hover:border-[#1a1456] hover:text-[#1a1456] transition-colors">
            {cat}
          </div>
        ))}
      </div>
      <div className="flex justify-center">
        <button className="flex items-center gap-2 border border-gray-300 text-gray-700 px-8 py-3 rounded-md text-[12px] font-medium hover:bg-gray-50">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        {steps.map((s) => (
          <div key={s.step} className="bg-white rounded-md p-6 flex flex-col gap-3">
            ...
            <h3 className="font-bold text-gray-800 text-[15px]">{s.title}</h3>
            <p className="text-gray-500 text-[12px]">{s.desc}</p>
          </div>
        ))}
      </div>
      <div className="flex justify-center">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {reviews.map((r) => (
          <div key={r.name} className="border border-gray-200 rounded-xl p-6 flex flex-col gap-4">
            ...
            <p className="text-gray-600 text-[12px] italic">"{r.text}"</p>
            <div className="flex items-center gap-3 mt-2">
              <div className="w-10 h-10 bg-orange-100 rounded-full flex items-center justify-center font-bold text-orange-600 text-[12px]">
                {r.initials}
              </div>
              <div>
                <p className="font-bold text-gray-800 text-[12px]">{r.name}</p>
