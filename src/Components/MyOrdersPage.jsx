  ...
  return (
    <div className="w-full gap-2  bg-[#eef2f7] min-h-screen flex mx-10">
      ...
      <div className="w-[220px] flex-shrink-0 bg-white border-r border-gray-200 p-5">
        <h2 className="font-bold text-[#1a1456] text-xl mb-4">Order History</h2>
        <h3 className="font-bold text-gray-700 text-sm mb-3">Filters</h3>

        {/* ORDER STATUS */}
        <div className="mb-4">
          <p className="text-gray-600 text-sm font-medium mb-2">Order Status</p>
          {["All Orders", "In Progress", "On the way", "Delivered"].map((s) => (
            <label key={s} className="flex items-center gap-2 py-1 cursor-pointer text-sm text-gray-600">
        ...
        {/* ORDER TIME */}
        <div>
          <p className="text-gray-600 text-sm font-medium mb-2">Order Time</p>
          {["Last 30 days", "2024", "2023"].map((t) => (
            <label key={t} className="flex items-center gap-2 py-1 cursor-pointer text-sm text-gray-600">
      ...
      <div className="flex-1 p-6 flex flex-col gap-4  mr-20 bg-white">
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1 flex items-center border border-gray-300 rounded-md bg-white px-4 h-11 gap-2">
            <input type="text" placeholder="Search your order here" className="flex-1 outline-none text-sm text-gray-600" />
            <FaSearch className="text-gray-400" />
          </div>
          <button className="bg-[#1a1456] text-white px-6 py-2.5 rounded-md text-sm font-medium hover:bg-[#2a2466] whitespace-nowrap">
            Download Selected
          </button>
        </div>
        ...
        <div className="bg-white rounded-t-lg">
          <div className="grid grid-cols-[40px_1fr_1fr_1fr_1fr_1fr] px-4 py-3 border-b border-gray-200">
            ...
            <span className="text-sm font-bold text-gray-700">Links & Documents</span>
            <span className="text-sm font-bold text-gray-700 text-right">Order Amount<br />(Inc GST)</span>
          </div>
        </div>
          ...
          {orders.map((order, index) => (
            <div key={index} className="bg-white rounded-lg border border-gray-200">
              ...
              <div className="grid grid-cols-[40px_1fr_1fr_1fr_1fr_1fr] px-4 py-4 items-center">
                ...
                <span className="font-bold text-gray-800 text-sm">{order.status}</span>
                <a href="#" className="text-[#1a1456] text-sm underline">View Tracking</a>
                <div className="flex items-center justify-end gap-2">
                  <span className="font-bold text-gray-800 text-sm">Total-INR {order.amount}</span>
              ...
              <div className="grid grid-cols-[40px_1fr_1fr_1fr_1fr_1fr] px-4 pb-4 items-center border-t border-gray-100">
                ...
                <span className="font-bold text-gray-700 text-sm">Logistics Docs</span>
